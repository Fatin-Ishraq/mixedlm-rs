...
# ------------------------------------------------------- freshness controls
def test_the_generator_files_are_watched_for_changes(record):
    """Both sweeps name their generator; both generators must be watched.

    A change to either changes the recorded counts without touching a line of
    the package, so freshness that covers only `python/` and `src/` would miss
    it. The names are read from the baseline itself rather than hard-coded,
    so moving a generator cannot quietly drop it from the rule.
    """
    watched = set(baseline_tests.NUMERIC_PATHS)
    for required in ("src", "python"):
        assert required in watched, f"{required} is not watched for freshness"

    files = set(baseline_tests.NUMERIC_FILES)
    for sweep in ("differential", "stress"):
        generator = record[sweep].get("generator")
        assert generator, f"{sweep} does not record its generator"
        path = generator.split("::")[0]
        assert path in files, (
            f"the {sweep} sweep is generated by {path}, which freshness does "
            f"not watch: a change to it moves the counts silently. Watched: "
            f"{sorted(files)}")
    assert "bench/baseline.py" in files, "the recorder itself is not watched"
