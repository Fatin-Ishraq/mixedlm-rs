...
| **measured against lme4** | `python bench/three_way.py` writes **`bench/three_way.json`**: all three packages on the same CSV files in one session, lme4 timed inside R. Recorded on the 0.2.0 build. |
| **historical** | the **`pymer4`** columns and the older `lme4` table below. `pymer4` needs `rpy2` and was not re-run; those rows are carried forward verbatim from the run named in `performance.json`, tagged with the environment that produced them, and were **not** re-measured on this build. |
...
|---:|---:|---:|:---:|---:|:---:|---:|---:|---:|---:|
| 2,000 | 100 | 0.35 s | True | **0.007 s** | True | **50x** | 6.6e-06 | 7.6e-05 | +1.2e-06 |
| 10,000 | 500 | 1.81 s | True | **0.008 s** | True | **215x** | 4.5e-07 | 1.4e-05 | +2.8e-07 |
| 20,000 | 1,000 | 2.67 s | True | **0.011 s** | True | **250x** | 3.4e-07 | 2.6e-05 | +1.1e-06 |
| 40,000 | 5,000 | 11.47 s | True | **0.014 s** | True | **814x** | 2.0e-06 | 1.8e-05 | +2.3e-06 |
| 100,000 | 20,000 | 43.36 s | True | **0.024 s** | True | **1777x** | 3.8e-05 | 4.3e-05 | +5.1e-05 |
| 200,000 | 50,000 | 108.35 s | True | **0.046 s** | True | **2375x** | 1.2e-05 | 3.7e-05 | +6.9e-05 |
| 500,264 | **125,066** | not run | — | **0.124 s** | True | — | — | — | — |

Recorded in `bench/performance.json` at the commit and machine named there,
three repetitions per case, minimum reported.

Recorded on 0.2.0. The 0.1.1 record at the same sizes gave 40x / 183x /
229x / 661x / 1092x / 1404x; the statsmodels column moved by under 10%, so
the change is this package's fit time (see "Against the previous release" in
the README and `bench/phases.json`).

> **Earlier figures were corrected in 0.1.1.** Revisions before it published
> 218x / 870x / 1454x / 1836x for the four middle rows of the 0.1.0 build.
> Those did not reproduce on that build and are superseded rather than
> explained; the history is in `RELEASE.md`.
...
`python bench/three_way.py` fits byte-identical CSV files with `mixedlm-rs`,
`statsmodels` and `lme4` in one session on the machine above. lme4 is timed
inside R with `Sys.time()`, from a data frame in memory to a fitted model, as
the Python packages are. Minimum of three repetitions. Each Python fit's
estimates are checked against lme4's (`bench/tolerances.py`) before its time is
kept, and the timing refuses to record if background CPU load exceeds 15%.
Recorded in `bench/three_way.json` at commit `7677222`.

| n | groups | lme4 | statsmodels | mixedlm-rs | vs lme4 | vs statsmodels |
|---:|---:|---:|---:|---:|---:|---:|
| 2,000 | 100 | 0.030 s | 0.351 s | **0.0075 s** | **4x** | 47x |
| 10,000 | 500 | 0.075 s | 1.739 s | **0.0083 s** | **9x** | 210x |
| 20,000 | 1,000 | 0.163 s | 2.636 s | **0.0130 s** | **13x** | 203x |
| 40,000 | 5,000 | 0.322 s | 11.041 s | **0.0160 s** | **20x** | 689x |
| 100,000 | 20,000 | 1.135 s | 43.134 s | **0.0273 s** | **42x** | 1578x |
| 200,000 | 50,000 | 2.409 s | 107.936 s | **0.0468 s** | **51x** | 2306x |
| 500,264 | 125,066 | 7.033 s | not run | **0.1100 s** | **64x** | — |

The ratio to lme4 grows with the group count, from 4x to 64x. From 10,000 to
40,000 rows it matches the historical table below (8x to 19x). From 100,000
rows up it is about 1.5x larger (42x, 51x and 64x against 30x, 32x and 43x),
which is where this release's own gains over 0.1.1 are largest.

**Accuracy against lme4**, on the 120 randomised fixtures of the differential
sweep (seeds 10000..10119, REML and ML, badly scaled predictors, near-zero
variances, unbalanced groups):

| outcome, relative to lme4's optimum | mixedlm-rs | statsmodels |
|---|---:|---:|
| matches lme4 | 113 | 74 |
| higher likelihood than lme4 | 7 | 3 |
| lower, and flagged not converged | 0 | 26 |
| lower, but reported converged | 0 | 17 |
| raised an error | 0 | 0 |

A fit "matches" when its deviance is within 1e-3 of lme4's. lme4 is a reference
here, not ground truth, because its optimiser can also stop short. That is why a higher
likelihood is counted separately rather than folded into "disagrees".

### Historical: lme4 and pymer4 on 0.1.0
...
> `historical`, on the 0.1.0 build. The `mixedlm-rs` column is from that same
> run, so the ratios are internally consistent; they are not a claim about the
> current build. The current build's lme4 comparison is the section above.
> `pymer4` needs `rpy2` and was not re-run.
